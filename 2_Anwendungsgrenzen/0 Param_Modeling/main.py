...
import Parametric_Modelling # type: ignore
...
def main(index=0):
    ...
    value_domain = Parameter.Parameterwertbereich_Mix
    ...
    param_set = parameter_space[index] #-> Gibt ein spezifisches Klassen-Objekt zurück
    # print(f"PARAM_SET [{index}]: ", param_set) # Demo
    log.info(f"Param_Set [{index}]:  {param_set}:")
    ...
    exporter.export_data_as_json(param_set_dic, "Parameterset", export_path, index)
    exporter.export_data_as_json(geometry_dic, "BuildingGeometry", export_path, index)
    exporter.export_data_as_ifc(ifc_model, "Parametrisches Modell", export_path, index)
    """Ende - Tests"""
...
if __name__ == "__main__":
    index = [0, 50, 55, 100, 111, 150, 155, 200, 222, 250]
    # index = [0]
